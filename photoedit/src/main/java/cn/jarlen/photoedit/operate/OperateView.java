...
public class OperateView extends View
{
	...
	private float picScale = 1f; //Changed by Danny
...
//						}
//						if (getSelected().isTextObject) {
//							myListener.onClick((TextObject) getSelected()); //Changed by Danny!
//						} else {
...
//						}
				...
				for (int i = 0; i < imgLists.size(); ++i)
				{
					...
					if (i != selectedId)
					{
						...
						if (getSelected() == null) {
							if (io.isTextObject) {
								myListener.onOutSideClick((TextObject) io);//Changed by Danny!
							} else {
			...
			case MotionEvent.ACTION_UP :
				...
				if (getSelected() != null) {
					if (getSelected().isTextObject) {
						myListener.onClick((TextObject) getSelected(), isMoveEvnt); //Changed by Danny!
					} else {
						myListener.onImageObjectClick(getSelected());
					}
				}
	...
	* */
	...
	public interface MyListener
	{
		...
		public void onOutSideClick(TextObject textObject);//Changed by Danny!
